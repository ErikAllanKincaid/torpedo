...
- `src/apply.rs` — declarative deploy spec for `ray apply` (Phase B): `DeploySpec = BTreeMap<network, NetworkSpec { trusted, firewall: SuggestedFirewall }>`, format-agnostic load via the `config` crate (YAML/TOML/JSON, auto-detected by extension; YAML is the documented default), `expected_hosts()` (union of subject + peer hostnames in the spec's firewall blocks), and the `EXAMPLE_SPEC` (YAML) printed by `--example`. The orchestrator itself lives in `main::ipc_apply`.
...
- **Declarative apply (`ray apply`):** reconcile trusted networks against a spec (`ray apply deploy.yaml`) — format-agnostic via the `config` crate (YAML by default, TOML/JSON by extension); a `networks:` map of `<name> → { trusted, firewall: SuggestedFirewall }`. The orchestrator (`main::ipc_apply`) fetches `Status` once, then for each spec network: `Create { trusted }` if absent (never joins), and publishes the spec's firewall block as suggestions (idempotent — always replaces the live set). `--prune` publishes exactly the spec's subjects, dropping out-of-band suggestions for hosts no longer mentioned; without it, spec subjects merge over the live set (so `apply` never silently drops a subject). `--dry-run` echoes the normalized spec as YAML; `--example` prints a template. **Membership diff (B3):** expected hosts = union of hostnames in the `firewall:` blocks; joined hosts = this node + peer hostnames from `Status`. The gap is reported as `ray invite <net> --hostname <missing>` commands; `--invite-missing` mints the one-time hostname-bound invites via IPC. No lock file; the live signed blob is state.
