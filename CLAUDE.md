...
- **Reachability model (segmentation-first):** a network is a reachability boundary — two peers can exchange packets iff they share ≥1 network (a QUIC connection only exists within a shared network, so this is enforced by connection existence). Coarse access is the network split itself; the per-device firewall is the fine-grained layer (directional, port-, and network-scoped). The ACL still works but is de-emphasized in favour of network split + firewall (it can't express direction or ports). Declarative provisioning of all this (`ray apply`) is a planned follow-up — see `TODO.md`.
- **ACL / firewall:** ACL is coordinator-managed, distributed in the GroupBlob, enforced at the routing layer (outbound uses *union* semantics across the networks shared with a multi-homed peer); firewall is per-device, first-match-wins, enforced after ACL. Paired devices resolve to one user identity via `DeviceUserMap`.
