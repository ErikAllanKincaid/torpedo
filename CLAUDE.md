...
- `src/dns.rs` — Magic DNS resolver: UDP+TCP DNS server on `127.0.0.1:53`, answers A, AAAA, PTR, and SOA queries for `*.pi` names from in-memory HostnameTable (network → hostname → `HostnameEntry = (Ipv4Addr, Ipv6Addr)`) and ReverseLookupTable (`DashMap<IpAddr, (hostname, network)>` for PTR records). EDNS/OPT support (echoes OPT with 1232-byte UDP size), SOA in NXDOMAIN authority section, NODATA for unsupported query types on `.pi` names, REFUSED for non-`.pi` queries. Concurrent query handling via `tokio::spawn` per packet. `spawn_dns_server(table, reverse, cancel)`, `update_hostname()` / `remove_hostname_by_ip()` / `remove_network()` helpers that update both forward and reverse tables atomically
- `src/dns_config.rs` — OS-level DNS configuration: `DnsConfigurator` trait with `apply()`/`revert()`, platform detection chain. macOS: SCDynamicStore with session keys (auto-cleanup on process exit), writes `State:/Network/Service/pitopi/DNS` with `ServerAddresses`, `SupplementalMatchDomains`, and `SearchDomains`. Linux detection chain: (1) systemd-resolved via D-Bus (`org.freedesktop.resolve1` SetLinkDNS/SetLinkDomains/RevertLink), (2) NetworkManager via D-Bus (detects DNS mode, configures when NM manages DNS directly), (3) systemd-resolved CLI fallback (`resolvectl`), (4) resolvconf with openresolv/Debian variant detection (`resolvconf --version`), (5) direct `/etc/resolv.conf` with backup/restore. D-Bus uses `zbus::blocking::Connection` (pure Rust, no C deps). `update_search_domains()` / `clear_search_domains()` for bare hostname resolution
