#!/usr/bin/env bash
# `torpedo connect` (direct 2-peer connection) e2e test orchestrator.
#
# Topology:
#   srv-a  identity U   the initiator (`torpedo connect`)
#   srv-b  identity V   the recipient (`torpedo connections approve`)
#
# Proves the full friend-request flow over real hosts + the public pkarr DHT:
#   B publishes a contact id  ->  A `torpedo connect <id>`  ->  B sees + approves
#   ->  a 2-peer `[direct]` network forms  ->  A<->B reach each other (ping +
#   torpedo send) and the network is tagged direct with its room id hidden.
...
# infra. Re-runnable (resets torpedo state on each run unless KEEP_STATE=1).
set -uo pipefail
...
A_CID="$(on "$A" 'torpedo contact id' | strip | head -1 | tr -d ' ')"
B_CID="$(on "$B" 'torpedo contact id' | strip | head -1 | tr -d ' ')"
...
# The contact id must also surface in `torpedo status`.
on "$A" 'torpedo status' | strip | grep -qi "${A_CID:0:16}" && pass "contact id shown in torpedo status" \
  || fail "contact id not shown in torpedo status"
...
CONNECT_OUT=""
for _ in $(seq 1 6); do
  CONNECT_OUT="$(on "$A" "torpedo connect $B_CID --hostname dario" 2>&1 | strip)"
...
REQ=""
for _ in $(seq 1 8); do
  REQ="$(on "$B" 'torpedo connections' 2>/dev/null | strip)"
...
APPROVE="$(on "$B" "torpedo connections approve $A_CID" 2>&1 | strip)"
...
# ---------------------------------------------------------------------------
step "7. data transfer — torpedo send / torpedo files accept (both directions)"
# `torpedo send` resolves the destination by hostname (or short id), not by IP.
...
[[ -n "$PEER_OF_A" ]] && send_recv "$A" "$B" "$PEER_OF_A" "torpedo send srv-a -> srv-b" || fail "could not resolve srv-b hostname"
[[ -n "$PEER_OF_B" ]] && send_recv "$B" "$A" "$PEER_OF_B" "torpedo send srv-b -> srv-a (reverse)" || fail "could not resolve srv-a hostname"
...
if [[ -n "$A_IP" && -n "$B_IP" ]]; then
  ...
  on "$B" 'torpedo firewall add in allow -p icmp' 2>&1 | strip | sed 's/^/   b| /'
...
if [[ -n "$A_IP" && -n "$B_IP" ]]; then
  ...
  on "$B" "torpedo firewall add in allow -p tcp --port $FWPORT" 2>&1 | strip | sed 's/^/   b| /'
...
# error, not hang.
on "$B" 'torpedo down' >/dev/null 2>&1 || true
sleep 3
...
NEW_B_CID="$(on "$B" 'torpedo contact rotate' 2>/dev/null | strip | grep -oE '[A-Za-z0-9]{20,}' | head -1)"
sleep 3
OFFLINE_OUT="$(on "$A" "torpedo connect ${NEW_B_CID:-$B_CID}" 2>&1 | strip)"
