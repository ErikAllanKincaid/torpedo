...
sudo cargo -q run -- install      # install/refresh the service and start it (explicit)
sudo cargo -q run -- restart      # restart the running service (e.g. after upgrading the binary)
sudo cargo -q run -- uninstall    # stop and remove the system service
sudo cargo -q run -- set-operator alice  # authorize user 'alice' to run ray without sudo
...
Only `install`, `restart`, `uninstall`, and `set-operator` (and the service-internal `daemon`) require `sudo`. `up`/`down` and all other commands run unprivileged via IPC.

**Access control (Tailscale's operator model):** the daemon authorizes each IPC request by the connecting socket's `SO_PEERCRED` UID rather than by file permissions. Read-only commands (`status`, `acl … show`, `firewall show`, `files`) are open to any local user; mutating commands require the caller to be **root** or the configured **operator UID**; setting the operator is root-only. `sudo ray set-operator <user>` records the operator UID in config, and `ray install` / `ray up` (when they install the service) auto-grant operator access to `$SUDO_USER`, so the installing user can run `ray` without sudo immediately. The IPC socket is mode `0666` (connectable by anyone — authority comes from the per-request UID check, not from reaching the socket).
...
- `src/config.rs` — persistent network config at `~/.config/rayfish/networks.toml` (members + approved list); `NetworkConfig` has `network_secret_key: Option<SecretKey>` (hex-serialized via custom serde adapter, coordinators only), `network_public_key: Option<EndpointId>` (the join code), and `my_hostname: Option<String>` (persisted so hostname survives daemon restarts); `AppConfig` has `operator_uid: Option<u32>` (the local UID authorized to control the daemon without root — Tailscale's operator model)
...
**Daemon/IPC:** `ray daemon` starts a long-lived root process that owns the iroh Endpoint, TUN device, PeerTable, and ProtocolRouter. CLI commands (`create`, `join`, `leave`, `nuke`, `status`, `down`) connect via Unix socket IPC (`/var/run/rayfish/rayfish.sock`) using length-prefixed JSON. The socket is mode `0666` (connectable by any local user); authority is enforced per-request in `DaemonState::check_authorized()` from the connection's `SO_PEERCRED` UID — reads are open, mutating commands need root or the configured `operator_uid`, and `SetOperator` is root-only (`DaemonState::set_operator()` persists it). This is Tailscale's `--operator` model: the always-root daemon does the privileged work, the client toggle is unprivileged once the user is the operator. The daemon uses `Endpoint::set_alpns()` to dynamically add/remove network ALPNs at runtime. Each active network registers a `MeshProtocol` handler (wrapping `CoordinatorAcceptState` or `MemberAcceptState` via `AcceptHandler` enum) with the ProtocolRouter and gets a `NetworkHandle` with a child `CancellationToken` for clean teardown on leave. `create` generates a per-network keypair and local alias; `join` accepts a public key string and resolves it via pkarr; `nuke` publishes empty record before leaving.
