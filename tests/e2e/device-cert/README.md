...
End-to-end test that a third peer can communicate, over a torpedo **closed**
...
IP (the multi-homed-peer model). The test pings and `torpedo send`s between srv-c and
...
- The run is fully re-runnable against the same `.servers`: it **resets each
  host's torpedo state** (stops the daemon, wipes `/root/.config/torpedo`) at the
...
> **Note on the device-cert restart.** `torpedo pair` stores the cert to disk but
...
- Servers are **left running** after the run so you can inspect them
  (`ssh root@<ip> torpedo status`). `tests/e2e.sh device-cert teardown` terminates
...
1. `torpedo pair` binds srv-b's transport key to srv-a's user identity.
2. All three peers get **distinct per-device IPs** (torpedo derives the IP from
...
4. Bidirectional `ping` over the TUN: srv-c↔srv-a and srv-c↔srv-b (C reaches
   both physical devices of identity U).
5. `torpedo send` round-trips (sha256-verified) srv-c→srv-a, srv-c→srv-b, and
   srv-a→srv-c.
...
- **Fixed — fresh-daemon ALPN gap (`src/daemon.rs`):** at startup the endpoint
  ...
  with no network rejected `torpedo pair`/`torpedo send` with "peer doesn't support any
...
- **Open — pairing doesn't refresh the in-memory device cert:** the
  ...
  daemon's `self.device_cert`, so a `torpedo pair` followed by `torpedo join` in the same
