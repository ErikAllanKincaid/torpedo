# `torpedo connect` direct-connection e2e

End-to-end test for the **direct 2-peer connection** flow (`torpedo connect`) over two
...
```
srv-a  identity U   initiator   (torpedo connect <B-contact-id>)
srv-b  identity V   recipient   (torpedo connections approve <id>)
```

## What it proves

1. Both nodes publish a **contact id** (`torpedo contact id`), also shown in `torpedo status`.
2. `torpedo connect <contact-id>` resolves the id via pkarr, dials, and queues as **pending**.
3. The recipient sees it in `torpedo connections` and `torpedo connections approve <id>`.
...
6. `torpedo send` / `torpedo files accept` round-trips a file both ways (sha256 verified).
...
state reset). The run is re-runnable — it resets torpedo state on each host
