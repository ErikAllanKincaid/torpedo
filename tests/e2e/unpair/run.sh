#!/usr/bin/env bash
# `torpedo unpair` (device-cert revocation) e2e test orchestrator.
#
...
#   A pairs B  ->  B joins A's closed network  ->  A `torpedo unpair srv-b`
#   ->  A bumps its cert generation and publishes the new floor (_torpedo_certgen),
...
[[ -n "$INV_B" ]] && on "$B" "torpedo join $INV_B" 2>&1 | strip | sed 's/^/   b| /'
[[ -n "$INV_C" ]] && on "$C" "torpedo join $INV_C" 2>&1 | strip | sed 's/^/   c| /'
wait_roster "$A" srv-b srv-c
SB="$(on "$B" 'torpedo status' | strip)"; SC="$(on "$C" 'torpedo status' | strip)"
...
# ---------------------------------------------------------------------------
step "3. srv-a lists its paired devices (torpedo pair list)"
PLIST="$(on "$A" 'torpedo pair list' | strip)"
echo "$PLIST" | sed 's/^/   a| /'
if echo "$PLIST" | grep -qi 'srv-b'; then
  pass "srv-b appears in 'torpedo pair list' on the primary"
else
  fail "srv-b not listed by 'torpedo pair list'"
fi

# ---------------------------------------------------------------------------
step "4. srv-a unpairs srv-b"
UNPAIR="$(on "$A" 'torpedo unpair srv-b' 2>&1 | strip)"
echo "$UNPAIR" | sed 's/^/   a| /'
if echo "$UNPAIR" | grep -qi 'revoked its device certificate'; then
  pass "srv-a unpaired srv-b"
else
  fail "torpedo unpair srv-b did not report success"
fi

# A secondary cannot unpair (primary-only guard).
SEC="$(on "$B" 'torpedo unpair srv-a' 2>&1 | strip || true)"
...
if on "$A" 'torpedo status --json' 2>/dev/null | strip | grep -qi 'srv-b'; then
...
REJOINED=0
for _ in $(seq 1 8); do
  on "$A" 'torpedo status --json' 2>/dev/null | strip | grep -qi 'srv-b' && { REJOINED=1; break; }
  sleep 5
done
...
if on "$B" 'test -f /etc/torpedo/device_cert'; then
