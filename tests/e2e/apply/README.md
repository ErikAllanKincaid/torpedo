...
Three hosts driving `torpedo apply` end to end against a live daemon: coordinator +
...
| Step | Coverage |
|------|----------|
| 1 | **Create + diff**: apply mints the missing closed `infra` (never joins) and reports the membership gap as `torpedo invite … --hostname srv-b/srv-c` lines. |
...
| 3 | **`torpedo identityof`**: prints a joined host's identity (and `--json` `paired=false`); errors for a host that hasn't joined. |
