...
| 4 | The resolver **binds no host `:53` socket** — it answers via the magic IP (`10.88.100.53`) routed through the TUN, so it coexists with any existing `:53` resolver (AdGuard/Pi-hole/dnsmasq — the umbrelOS case) by construction. |
